export default function attachChatBox(
  onReceiveChat: (element: HTMLElement) => void
) {
  ...
  const observer = new MutationObserver((records, observer) => {
    ...
    for (const record of records) {
      for (const item of Array.from(record.addedNodes)) {
        ...
        if (
          element?.localName === "yt-live-chat-text-message-renderer" ||
          element?.localName === "yt-live-chat-paid-message-renderer"
        ) {
